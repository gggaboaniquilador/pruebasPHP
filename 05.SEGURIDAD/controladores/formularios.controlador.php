...
class ControladorFormularios{
	...
	static public function ctrRegistro(){
		...
		if(isset($_POST["registroNombre"])){
			...
			$encriptarPassword = crypt($_POST["registroPassword"], '$2a$07$jesusgabrielperezperez$');
...
#SELECCIONAR REGISTROS
	...
	public function ctrIngreso(){
		if (isset($_POST["ingresoEmail"])){
			...
			$encriptarPassword = crypt($_POST["ingresoPassword"], '$2a$07$jesusgabrielperezperez$');
			
			$respuesta=ModeloFormularios::mdlSeleccionarRegistros($tabla, $item, $valor);
			if ($respuesta["email"]==$_POST["ingresoEmail"]&&$respuesta["password"]==$encriptarPassword) {

				ModeloFormularios::mdlActualizarIntentosFallidos($tabla,null,$respuesta["token"]);
			...
			if ($compararToken == $_POST["tokenUsuario"] && $_POST["idUsuario"]==$usuario["id"]) {
				...
				if ($_POST["actualizarPassword"]!="") {
					...
					if (preg_match('/^[0-9a-zA-Z]+$/', $_POST["actualizarPassword"])) {
						$password = crypt($_POST["actualizarPassword"], '$2a$07$jesusgabrielperezperez$');
					}
